...
import { ChevronLeft, Clock, Flame, Hourglass, Repeat, Timer } from 'lucide-react-native';
      ...
      <ScrollView className="flex-1 p-5">
        ...
        <View className="flex-row flex-wrap gap-4">
          {timerOptions.map((option) => (
            ...
            >
              <View className={`w-12 h-12 ${option.iconBg} rounded-full items-center justify-center`}>
                <option.icon size={24} color={option.iconColor} />
              </View>
              <View className="items-center">
                <Text className="font-montserrat-bold text-gray-900">{option.title}</Text>
                <Text className="text-xs text-gray-400 font-montserrat">{option.subtitle}</Text>
              </View>
            </TouchableOpacity>
          ))}
        </View>
